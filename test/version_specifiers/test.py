...
def test_wildcard_latest():
    ...
    pri_txt = (proj / (".package.lua" if _G_PACK_TYPE == "xmake" else (".package.cmake" if _G_PACK_TYPE == "cmake" else ".package.pri"))).read_text() if ((proj / (".package.lua" if _G_PACK_TYPE == "xmake" else (".package.cmake" if _G_PACK_TYPE == "cmake" else ".package.pri")))).exists() else ""
...
def test_version_skip_x():
    ...
    r = _run(proj)
    _check(r.returncode == 0, f"rc={r.returncode}")
    if ((proj / (".package.lua" if _G_PACK_TYPE == "xmake" else (".package.cmake" if _G_PACK_TYPE == "cmake" else ".package.pri")))).exists():
        _check("hello" not in (proj / (".package.lua" if _G_PACK_TYPE == "xmake" else (".package.cmake" if _G_PACK_TYPE == "cmake" else ".package.pri"))).read_text(), "x should skip")
