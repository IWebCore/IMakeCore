...
namespace detail{
    ...
    static std::map<IString, IString>& getUserDefinedSuffixes(){
        static std::map<IString, IString> suffixes{};
        return suffixes;
    }
}
...
IStringView IHttpMimeUtil::getSuffixMime(const IString &suffix)
{
    ...
    if(detail::getUserDefinedSuffixes().find(suffix) != detail::getUserDefinedSuffixes().end()){
        return detail::getUserDefinedSuffixes()[suffix];
    }
...
void IHttpMimeUtil::registerSuffixMime(IString suffix, IString mime)
{
    suffix.solidify();
    mime.solidify();
    detail::getUserDefinedSuffixes()[std::move(suffix)] = std::move(mime);
}
