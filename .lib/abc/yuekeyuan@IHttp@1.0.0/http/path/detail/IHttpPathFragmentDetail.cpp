...
IWebCore::IHttpPathFragmentDetail::IHttpPathFragmentDetail(const QString& fragment)
{
    ...
    IHttpAbort::abortHttpPathFragmentError(info, $ISourceLocation);
}
...
bool IHttpPathFragmentDetail::evaluateNameTypeNode()
{
    ...
    if(valueType.isEmpty()){
        IHttpAbort::abortHttpPathFragmentError("no capture function name found", $ISourceLocation);
    }
    ...
    if(fun != nullptr){
        ...
        IHttpAbort::abortHttpPathFragmentError("not supported path function", $ISourceLocation);
    }
    return false;
}
...
bool IHttpPathFragmentDetail::evaluateRegTypeNode()
{
    ...
    if(!regExp.isValid()){
        IHttpAbort::abortHttpPathFragmentError("expression not valid", $ISourceLocation);
    }
